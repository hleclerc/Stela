...
Prop:
a := 10 # set_val( room_0( 32 ), cst( 10 ) )
a = 20
  # set_val( set_val( room_0( 32 ), cst( 10 ) ), cst( 20 ) )
  # avec RefItem::reassign() ?
    -> 
slice( a, beg, end ) # slice( set_val( room_0( 32 ), cst( 10 ) ), ... )
  -> permet de rester avec les Expr
Var a( ... )
a = 20 #  
p := &a # pointer_on( room_0() )
a = 10
@p # return l'expr en l'état (set_val( room_0( 32 ), cst( 10 ) ))
   # 
...
A-t-on besoin des types dans Room (et dans Expr) ?
- serait bien utile pour la destruction
- on pourrait aussi les mettre dans les Inst

A-t-on besoin des nout dans les Expr ?
- si plusieurs sorties, on pourrait renvoyer un agrégat
   -> pour le while, (if, et syscall)
        a := 10
        while ...
           a += 20
        # a = set_val( room(), slice( while(), ... ) )
