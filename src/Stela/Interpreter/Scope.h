...
public:
    ...
    Var parse( const PI8 *sf, const PI8 *tok );
...
protected:
    ...
    #define DECL_IR_TOK( N ) Var parse_##N( const PI8 *sf, int off, BinStreamReader bin ); ///< parse a given IR_TOK
    #include "../Ir/Decl.h"
    #undef DECL_IR_TOK

    Var parse_CALLABLE( const PI8 *sf, int off, BinStreamReader bin, Var *type );

    template<class T> Var make_var( T val );
    int read_nstring( const PI8 *sf, BinStreamReader &bin );
    Var copy( const Var &var, const PI8 *sf, int off );
    Var get_val_if_GetSetSopInst( const Var &val );
    Expr simplified_expr( const Var &var );
    Var apply( const Var &f, int nu, Var *u_args, int nn, int *n_names, Var *n_args, ApplyMode am, const PI8 *sf, int off );

    ErrorList::Error &make_error( String msg, const PI8 *sf = 0, int off = 0, bool warn = false );
    Var disp_error( String msg, const PI8 *sf = 0, int off = 0, bool warn = false );
    void set( Var &o, Expr n, const PI8 *sf, int off );
    Var reg_var( int name, const Var &var, const PI8 *sf, int off, bool stat = false, bool check = true );
    ...
    Var get_attr( Var self, int attr, const PI8 *sf, int off );
    Var *self_var();
