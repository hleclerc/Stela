...
static Expr _simp_op( Expr a, Expr b, Op_or_boolean ) {
    if ( a == b )
        return a;
    if ( a->always( true  ) ) return a;
    if ( b->always( true  ) ) return b;
    if ( a->always( false ) ) return b;
    if ( b->always( false ) ) return a;
    if ( not_boolean( a ) == b ) return true;
    return Expr();
}
