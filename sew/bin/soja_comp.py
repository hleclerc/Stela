            ...
            if r:
                ...
                if len( img ):
                    ...
                    for mime, ext in cmime:
                        for e in ext:
                            if img.endswith( e ) or img.endswith( e.upper() ):
                                kin = mime
                                break
                    if len( kin ):
...
def main():
    ...
    for f in fs.file_list:
        ...
        elif f.full_name.endswith( '.js' ):
            bout = os.path.basename( f.full_name )
            fout = os.path.join( args.d, bout )
            print fout
            if os.path.exists( fout ) == False or os.path.getmtime( f.full_name ) > os.path.getmtime( fout ):
                exec_cmd( "cp '" + f.full_name + "' '" + fout + "'" )
            js_files.append( bout )
        elif f.full_name.endswith( '.css' ):
            bout = os.path.basename( f.full_name )
            out = file( os.path.join( args.d, bout ), "w" )
            inp = file( f.full_name )
            fs.copy_with_inline_images( out, inp, f.full_name )
            css_files.append( bout )
            
    # execute
    if args.e:
        if len( js_files ) > 1:
            out = os.path.join( args.d, "out.js" )
            print string.join( [ os.path.join( args.d, j ) for j in js_files ] )
